...
def action():
	...
	# def kabita():
	# 	print(f"Your current balance is Rs.{Information(Amount).get()}")
	def clear():
		ac.delete(0,END)
		name.delete(0,END)
		amount.delete(0,END)
	...
	if ac.get()=="":
		...
		for i in Information.items():
			if int(ac.get()) == Information['Account'] and len(ac.get()) == 14:
				...
				Radio_button_vah = Radiobutton(des,text=' withdraw salary', value="withdraw",font = 'sans-serif 10 bold',variable = var)
				Radio_button_vah .place(x= 220 , y= 350)

				Radio_button_pro = Radiobutton(des,text='Enquiry salary', value="Enquiry",font = 'sans-serif 10 bold',variable = var)
				Radio_button_pro .place(x= 220 , y= 380)

				Radio_button_others = Radiobutton(des,text='Customer Details', value="Details",font = 'sans-serif 10 bold',variable = var)
...
a=Frame(des,height=1,width=240,bg="light blue")
a.place(x=880,y=120)

b=Frame(des,height=330,width=1,bg="light blue")
b.place(x=880,y=120)

c=Frame(des,height=1,width=240,bg="light blue")
c.place(x=880,y=450)

d=Frame(des,height=330,width=1,bg="light blue")
